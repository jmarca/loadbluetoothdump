...
function create_perlhash_statement(table,fk){
    var create= "CREATE TEMP TABLE "+table+" (data text,ts timestamp with time zone not null, radar_lane_id integer not null, station_lane_id integer not null,"
    var constraint
    if(fk){
...
function bt_parser(reader,writer){
    ...
    this.perl_parser=function(client,callback){
        ...
        var insert_statements = [
            'insert into bt_xml_project  (select distinct * from perl_xml_project_decoder_from_location())'
            ,'insert into bt_xml_location  (select distinct * from perl_xml_location_decoder_from_location())'
