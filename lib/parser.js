...
function parse_xml(xml){
    // parse xml, populate the hash
    var encoding = 'utf-8'
    logger.debug(xml)
    logger.debug(unescape(xml))
    // it is perl hash, not xml
    // var xmlDoc = libxmljs.parseXml(unescape(xml),encoding)
    logger.debug(JSON.stringify(xmlDoc))
    return null
}
